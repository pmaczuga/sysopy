...
{
    if(buffer->size == buffer->elements) return 1;
    return 0;
}

int buffer_empty(struct Buffer *buffer)
{
    if(buffer->elements == 0) return 1;
    return 0;
}
...
void clean_up()
{
    if(pthread_mutex_destroy(&mutex_memory) == -1)
        perror("ERROR: Problem destroying mutex: mutexMemory");
    if(pthread_cond_destroy(&cond_empty) == -1)
        perror("ERROR: Problem destroying condition variable: cond_empty");
    if(pthread_cond_destroy(&cond_full) == -1)
        perror("ERROR: Problem destroying ondition variable: cond_full");
    if(pthread_cond_destroy(&cond_eof) == -1)
        perror("ERROR: Problem destroying ondition variable: cond_eof");
}
...
void *producer(void *arg)
{
    ...
    while(1)
    {
        ...
        while(buffer_full(&buffer) == 1)
        {
            pthread_cond_wait(&cond_full, &mutex_memory);
        }
...
void *consumer(void *arg)
{
    ...
    {
        pthread_mutex_lock(&mutex_memory);
        while(buffer_empty(&buffer) == 1)
        {
            pthread_cond_wait(&cond_empty, &mutex_memory);
        }
...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr, "ERROR: Problem with initizalizng buffer\n");
        exit(EXIT_FAILURE);
    }

    // Initialize mutexes and condintion variables
    mutex_memory = (pthread_mutex_t) PTHREAD_MUTEX_INITIALIZER;

    cond_full = (pthread_cond_t) PTHREAD_COND_INITIALIZER;
    cond_empty = (pthread_cond_t) PTHREAD_COND_INITIALIZER;
    cond_eof = (pthread_cond_t) PTHREAD_COND_INITIALIZER;
    ...
    if(params.nk > 0)
    {
        ...
        while(eof == 0)
        {
            int res = pthread_cond_timedwait(&cond_eof, &mutex_memory, &ts);
            if(res == ETIMEDOUT)
            {
                printf("Time passed\n");
                break;
    ...
    else
    {
        ...
        while(eof == 0)
        {
            pthread_cond_wait(&cond_eof, &mutex_memory);
        }
        pthread_mutex_unlock(&mutex_memory);
        printf("End of file reached\n");
    }

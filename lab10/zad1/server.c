...
int add_client(char *name, int fd, struct sockaddr addr)
{
    ...
    clients[destination]->addr_len = sizeof(addr);
    return OK;
}
...
void *task_network(void *args)
{
    ...
    while(1)
    {
        ...
        if(received_events > 0)
        {
            ...
            client_fd = accept(events[0].data.fd, &client_addr, &addr_len);
            if(client_fd == -1)
            {
            ...
            if(recv(client_fd, client_name, 64, 0) == -1)
                perror("ERROR: Problem receiving name");
            else
            {
                // message sent to client - either NAME_TAKEN, LIST_FULL or OK
                int int_message;
                sem_wait(&sem_memory);
                int_message = add_client(client_name, client_fd, client_addr);
                sem_post(&sem_memory);
                ...
                if(send(client_fd, &int_message, sizeof(int), 0) == -1)
        ...
        if(sem_trywait(&sem_input) == 0)
        {
            ...
            sem_wait(&sem_ping);
            if(client == NULL || send(client->fd, &my_message, sizeof(my_message), MSG_NOSIGNAL) == -1)
...
void *task_ping(void *args)
{
    ...
    while(1)
    {
        ...
        while(i < MAX_CLIENTS)
        {
            ...
            if(clients[i] != NULL)
            {
                if(send(clients[i]->fd, &my_message, sizeof(my_message), MSG_NOSIGNAL) == -1)
                    to_remove = 1;
                if(recv(clients[i]->fd, &reply, sizeof(reply), 0) == -1)
                    to_remove = 1;
                if(to_remove == 1)
                {
...
int main(int argc, char *argv[])
{
    ...
    // create local socket
    fd_local = socket(AF_UNIX, SOCK_STREAM, 0);
    if(fd_local == -1)
    {
    ...
    fd_network = socket(AF_INET, SOCK_STREAM, 0);
    if(fd_network == -1)
    {
    ...
    // listen - second arguemnt tells how much requests can wait in queue
    listen(fd_local, MAX_CLIENTS);
    listen(fd_network, MAX_CLIENTS);
